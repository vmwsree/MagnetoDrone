...
def shapedef(pSkelt, index,SKEL_CORD):
    ...
    for i in range(0,len(SKEL_CORD)-1):
        ...
        curend = skeleton_to_depth_image(next, dispInfo.current_w, dispInfo.current_h)
        screen_draw(cursd,curstart[0],curstart[1])
        
        if(cursd>cured):
            kw='b'
        elif(cursd<cured):
            kw='f'
        if (abs(cursd-cured)<5):
            kw='s'
        depstr=depstr+kw
...
def surface_to_array(surface):
   ...
   return bytes
def screen_draw(stri,x,y,font=38):
...
def draw_skeletons(skeletons):
    ...
    isTracked=False
    finleng =40
    global past_pos,pdemor,pdemol,pdemo2,pdemo1,histl,histr,Nowgest
    ...
    for index, data in enumerate(skeletons):
                    if data.eTrackingState == nui.SkeletonTrackingState.TRACKED and not(isTracked):
                        ...
                        if len(histl)>10:
                            histr=histr[len(histr)-8:len(histr):1]
                            histl=histl[len(histl)-8:len(histl):1]
                        Just=1
                        
                        
                        screen_draw(depstr,ElbowPos[0],ElbowPos[1]+50)
                        #screen_draw(pdemor,HandPos[0],HandPos[1]+50)
                        #finlen =(born_len/finleng) *finleng
                        #if shapR==demor:
                        LastAction=0
                        FindGesture(shapL,shapR,ElbowPos,HandPos)

       

                            
def FindGesture(shapL,shapR,ElbowPos,HandPos):
    global pdemor,pdemol,histl,histr,Nowgest
    if len(histl)>7:
        if (histl[len(histl)-6:len(histl):1] == ['ldru','ldrd','ldru','ldrd','ldru','ldrd']) and (histr[len(histr)-6:len(histr):1] == ['rdld','rdlu','rdld','rdlu','rdld','rdlu']):
            Nowgest="Roll on"
