...
def draw_skeletons(skeletons):
    ...
    for index, data in enumerate(skeletons):
                    ...
                    if data.eTrackingState == nui.SkeletonTrackingState.TRACKED and index==lowst:
                        isTracked=True
                        dephead=int(data.SkeletonPositions[JointId.Head].z * 100)
                        isTracked=True
                        ...
                        if arm:
                            if not(Flying) :
                             ...
                             #++++++++++++++++++++++++++++++++++++++++++++++The Remote , Need to impliment based on Continues Gestures than stattic one++++++++++++++          
                                ...
                                if ((shapR=='dssrdf')or (shapR=='rdsrdf') or (shapR=='rdsrds')) :
                                    if ActionPerforming['MoveRight']==False: 
                                      drn.move_right()
                                      resetflag('MoveRight') 
                                elif ( (shapR=='dsslsf')or shapR=='dsslss') :
                                    if ActionPerforming['MoveLeft']==False: 
                                      drn.move_left()
                                      resetflag('MoveLeft') 
                                elif ((shapR=='ussuss') ) :
                                    if ActionPerforming['MoveUp']==False: 
                                      drn.move_up()
                                      resetflag('MoveUp') 
                                elif ((shapR=='rsfusf') and (shapL=='lsfusf')) : 
                                      if ActionPerforming['MoveForward']==False:
                                          drn.move_forward()
                                          resetflag('MoveForward')   
                                elif ((shapR=='rdsusf') and (shapL=='ldsusf')):
                                    if ActionPerforming['MoveForward']==False:
                                          drn.move_backward()
                                          resetflag('MoveBackward')  
                                elif ( (shapL=='lsslss')) :
                                    if ActionPerforming['TurnRight']==False: 
                                      drn.turn_right()
                                      resetflag('TurnRight') 
                                elif ( (shapL=='rssrss')) : 
                                    if ActionPerforming['TurnLeft']==False:  
                                      drn.turn_left()
                                      resetflag('TurnLeft')  
                                else:
                                    if ActionPerforming['Hover']==False:
                                        drn.hover()
                                        resetflag('Hover')
